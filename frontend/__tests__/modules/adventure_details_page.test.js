...
describe("Adventure Detail Page Tests", function () {
  ...
  it("Check if conditional rendering is working", function () {
    ...
    // CRIO_SOLUTION_START_MODULE_ADVENTURE_DETAILS
    // FIXME - See if we can re-write the tests to check for presence of element instead of checking CSS directly
    // CRIO_SOLUTION_END_MODULE_ADVENTURE_DETAILS
    expect(
  ...
  // FIXME - Add more finer tests for POST API call
  ...
  it("captureFormSubmit() - Makes a POST API call with correct request body", function () {
    ...
    // CRIO_SOLUTION_START_MODULE_RESERVATIONS
    // FIXME - Figure out the tests
    // Test if alert is shown with correct message
    // expect(window.alert).toHaveBeenCalledTimes(1);
    // expect(window.alert).toHaveBeenCalledWith(expect.stringMatching("/success/i"));

    // Test if window was reloaded
    // expect(window.location.reload).toHaveBeenCalledTimes(1);
    /*
    [
      'http://localhost:8082/reservations/new',
      {
        method: 'POST',
        body: '{"name":"","date":"","person":"0"}',
